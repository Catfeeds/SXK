...
public class ConfigCacheUtil {
	...
	public static String getUrlCache(String url, ConfigCacheModel model) {
		...
		String result = "";
		File file = new File(DirManager.getDataCache(), StringUtils.md5(url));
		if (file.exists() && file.isFile()) {
			...
			// 1。如果系统时间是不正确的
			// 2。当网络是无效的,你只能读缓存
			if (!NetUtils.isNetworkConnected(AppConfig
	 ...
	 * @return 缓存数据
	 */
	public static String getReadCache(String url, ConfigCacheModel model) {
		if (url == null) {
			return null;
		}

		String result = "";
		File file = new File(DirManager.getReadCache(), StringUtils.md5(url));
		if (file.exists() && file.isFile()) {
			long expiredTime = System.currentTimeMillis() - file.lastModified();
			Log.d(TAG, file.getAbsolutePath() + " expiredTime:" + expiredTime
					/ 60000 + "min");
	...
	public static void setUrlCache(String data, String url) {
		if (DirManager.getAppRoot() == null) {
			return;
		}
		String path = DirManager.getCachePath();
	 ...
	 * @param url
	 */
	public static void setReadUrlCache(String data, String url) {
		if (DirManager.getAppRoot() == null) {
			return;
		}
		String path = DirManager.getCachePath();
